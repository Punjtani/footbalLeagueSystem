...
class UserController extends BaseController
{
    ...
    {
        //    dd(User::where('user_type','user')->first());
        //    ->with(['roles','stadium'])
        try {
            if (request()->ajax()) {
                return datatables(User::filters(request())->where('user_type', 'user')->with(['roles', 'stadium'])
                ->orderBy('created_at','desc')->where('id', '!=', 1))->addColumn('actions', static function ($data) {
                    ...
                    ->addColumn('paymentStatus', function ($data) {
                        $subscription = Subscription::whereUserId($data->id)->first();
                    ...
                    ->addColumn('subscriptionId', function ($data) {
//                        $subscription = Subscription::whereUserId($data->id)->first();
//                        if (empty($subscription)) {
//                            return "---";
//                        }
                    ...
                    ->rawColumns(['actions', 'displayStatus'])
                    ->make(true);
            }
        } catch (Exception $ex) {

        }
        return view('pages.users.list', ['add_url' => route('users.create'), 'advance_filters' => $this->advance_filters(), 'breadcrumbs' => Breadcrumbs::generate('users')]);
    }
    ...
    public function create()
    {
        ...
        return view('pages.users.add', ['title' => 'Add User', 'tenants' => $tenants, 'stadiums' => $stadiums, 'breadcrumbs' => Breadcrumbs::generate('users.create')]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($checkUser !== NULL) { // non unique user, throw error
            ...
            return Helper::jsonMessage($user->id !== null, $index_url);
        }
    }
    ...
    public function edit($id)
    {
        ...
        $tenants = Tenant::query()->where('status', Tenant::STATUS_PUBLISH)->get();
        $stadiums = Stadium::where('status', 1)->pluck('name', 'id');
        return view('pages.users.add', ['title' => 'Edit User', 'tenants' => $tenants, 'stadiums' => $stadiums, 'item' => $admin, 'breadcrumbs' => Breadcrumbs::generate('users.edit', $admin)]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return Helper::jsonMessage($user !== null, $index_url, $user !== null ? 'Record Successfully Updated' : 'Unable to update');
    }
    ...
    public function getProfile()
    {
        ...
        return view('pages.admins.editProfile', ['admin' => $tenant, 'breadcrumbs' => Breadcrumbs::generate('users.profile')]);
    }
